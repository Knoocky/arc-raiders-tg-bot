...
from app.infrastructure.persistence.models import EventCatalogModel
...
class EventCatalogRepository:
    ...
    async def upsert_many(self, definitions: Sequence[EventDefinitionDTO]) -> list[EventDefinition]:
        now = utc_now()
        async with self._session_factory() as session:
            async with session.begin():
                rows = list((await session.execute(select(EventCatalogModel))).scalars())
    ...
    async def list_active(self) -> list[EventDefinition]:
        async with self._session_factory() as session:
            stmt = select(EventCatalogModel).where(EventCatalogModel.is_active.is_(True)).order_by(
                EventCatalogModel.display_name.asc()
            )
    ...
    async def list_all(self) -> list[EventDefinition]:
        async with self._session_factory() as session:
            stmt = select(EventCatalogModel).order_by(EventCatalogModel.display_name.asc())
            rows = list((await session.execute(stmt)).scalars())
            return [self._to_domain(row) for row in rows]

    async def get_by_id(self, catalog_id: int) -> EventDefinition | None:
        async with self._session_factory() as session:
            row = await session.get(EventCatalogModel, catalog_id)
    ...
    async def resolve_catalog_id(
        ...
        *,
        ...
        async with self._session_factory() as session:
            predicates = [EventCatalogModel.canonical_name == canonical_name]
            if external_id:
                predicates.insert(0, EventCatalogModel.external_id == external_id)
            stmt = select(EventCatalogModel).where(or_(*predicates)).order_by(EventCatalogModel.id.asc())
            rows = list((await session.execute(stmt)).scalars())
            if not rows:
                return None
            if external_id:
    ...
    @staticmethod
    def _to_domain(row: EventCatalogModel) -> EventDefinition:
